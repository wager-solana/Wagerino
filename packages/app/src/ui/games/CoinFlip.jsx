...
export function Canvas({ result, phase, state }) {
  ...
  const [face, setFace] = useState(state.side);
  useEffect(() => { if (phase && phase.step === "oracle") setSpin((s) => s + 1080); }, [phase?.step]);
  useEffect(() => { if (result) { const won = result.multX > 0; const landed = won ? result.state.side : result.state.side === "heads" ? "tails" : "heads"; setFace(landed); setSpin((s) => s - (s % 360) + 1800 + (landed === "tails" ? 180 : 0)); } }, [result]);
  useEffect(() => { if (!result && !phase) { setFace(state.side); setSpin((s) => s - (s % 360) + (state.side === "tails" ? 180 : 0)); } }, [state.side]);
  return <div className="coin" style={{ transform: `rotateY(${spin}deg)` }}><div className="f heads">★</div><div className="f tails">♛</div></div>;
}
