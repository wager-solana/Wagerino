...
export default function Home() {
  const { feed, games, platform } = useApp();
    ...
    <>
      ...
      <div className="promos">
        ...
        <div className="promo" style={{ background: "linear-gradient(140deg,#1f3a8a,#4f6fdc)" }}>
          <small>3 · Be the house</small><h2>Buy shares of any game. The edge is yours.</h2><a href="#/game/plinko">Shares →</a>
        </div>
      </div>
      <div className="section-h">∞ ORIGINALS</div>
      <div className="grid">
        {CATALOG.map((g) => (
          <a key={g.slug} href={`#/game/${g.slug}`} className="tile" style={{ background: `radial-gradient(circle at 50% 110%, ${g.color}55, var(--panel) 65%)`, opacity: has(g) ? 1 : 0.55 }}>
            ...
            <div className="art"><GameArt slug={g.slug} /></div>
